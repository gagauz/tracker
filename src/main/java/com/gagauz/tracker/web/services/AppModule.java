...
public class AppModule {
	...
	public static void contributeApplicationDefaults(MappedConfiguration<String, Object> configuration) {
		...
		configuration.override(ComponentParameterConstants.GRID_TABLE_CSS_CLASS,
				"table table-responsive valign-center");
		...
		return c -> {
			...
			return Cast.of(c)
					.castF(UsernamePasswordCredentials.class, credentials -> {
						return accountDao.findByNameAndPass(credentials.getUsername(),
								CryptoUtils.createSHA512String(credentials.getPassword()));
					})
	...
	public static CookieCredentialEncoder<User>
			buildCookieCredentialEncoder(@Inject final CookieEncryptorDecryptor cookieEncryptorDecryptor) {
		return user -> {
		...
		configuration.add("SetUserInSessionHandler", new AuthenticationHandler() {
			...
			public void handleLogin(LoginResult loginResult) {
				...
				if (loginResult.isSuccess() && null != user) {
					PrincipalStorage storage = Optional
							.ofNullable(applicationStateManager.getIfExists(PrincipalStorage.class))
