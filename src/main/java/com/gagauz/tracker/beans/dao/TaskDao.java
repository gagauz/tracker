...
public class TaskDao extends AbstractDao<Integer, Task> {
    ...
    public List<Task> findByProject(Project project) {
        return getSession().createQuery("from Task t where featureVersion.feature.project=:project").setEntity("project", project).list();
    }

    public List<Task> findByVersion(Version version) {
        return getSession().createQuery("from Task t where featureVersion.version=:version").setEntity("version", version).list();
    }

    public List<Task> findByFeature(Feature feature) {
        return getSession().createQuery("from Task t where featureVersion.feature=:feature").setEntity("feature", feature).list();
    }

    public List<Task> findOpen(Version version) {
        return getSession()
                .createQuery("from Task t where featureVersion.version=:version and status in (:status) order by t.progress/t.estimate desc priority desc")
